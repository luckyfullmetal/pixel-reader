...
print("⚙️ Video Matrix Encoder initialized (512-frame chunk mode - ALL FRAMES).")

print("🧹 Cleaning directory... Removing old .json files.")
deleted_count = 0
for file in os.listdir('.'):
    if file.endswith('.json') and not file.endswith('_audio.json'):
        try:
            os.remove(file)
            deleted_count += 1
...
def process_video(file_path):
    ...
    while True:
        ...
        # Process every single frame natively (Skipping line removed!)
